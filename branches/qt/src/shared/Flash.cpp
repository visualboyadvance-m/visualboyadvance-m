...
{
    //  winlog("Setting flash size to %d\n", size);
    if ( size == 0x10000 ) {
        flashDeviceID = 0x1b;
...
{
    //  winlog("Reading %08x from %08x\n", address, reg[15].I);
    //  winlog("Current read state is %d\n", flashReadState);
    address &= 0xFFFF;
...
{
    //  winlog("Writing %02x at %08x\n", byte, address);
    //  winlog("Current state is %d\n", flashState);
    address &= 0xFFFF;
    switch ( flashState ) {
    case FLASH_READ_ARRAY:
