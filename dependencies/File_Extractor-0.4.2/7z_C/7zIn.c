...
static SZ_RESULT MySzInAlloc(void **p, size_t size, void * (*allocFunc)(size_t size))
{
	if (size == 0)
		*p = 0;
	else
...
static SZ_RESULT SzArDbExFill(CArchiveDatabaseEx *db, void * (*allocFunc)(size_t size))
{
	UInt32 startPos = 0;
...
static SZ_RESULT SafeReadDirect(ISzInStream *inStream, Byte *data, size_t size)
{
	#ifdef _LZMA_IN_CB
	while (size > 0)
	{
...
static SZ_RESULT SafeReadDirectByte(ISzInStream *inStream, Byte *data)
{
	return SafeReadDirect(inStream, data, 1);
}

static SZ_RESULT SafeReadDirectUInt32(ISzInStream *inStream, UInt32 *value)
{
	int i;
	*value = 0;
	for (i = 0; i < 4; i++)
...
static SZ_RESULT SafeReadDirectUInt64(ISzInStream *inStream, UInt64 *value)
{
	int i;
	*value = 0;
	for (i = 0; i < 8; i++)
...
static SZ_RESULT SzReadByte(CSzData *sd, Byte *b)
{
	if (sd->Size == 0)
		return SZE_ARCHIVE_ERROR;
	sd->Size--;
...
static SZ_RESULT SzReadBytes(CSzData *sd, Byte *data, size_t size)
{
	size_t i;
	for (i = 0; i < size; i++)
	{
...
static SZ_RESULT SzReadUInt32(CSzData *sd, UInt32 *value)
{
	int i;
	*value = 0;
	for (i = 0; i < 4; i++)
...
static SZ_RESULT SzReadNumber(CSzData *sd, UInt64 *value)
{
	Byte firstByte;
	Byte mask = 0x80;
	int i;
...
static SZ_RESULT SzReadSize(CSzData *sd, CFileSize *value)
{
	UInt64 value64;
...
static SZ_RESULT SzReadNumber32(CSzData *sd, UInt32 *value)
{
	UInt64 value64;
	RINOK(SzReadNumber(sd, &value64));
...
static SZ_RESULT SzReadID(CSzData *sd, UInt64 *value)
{
	return SzReadNumber(sd, value);
}

static SZ_RESULT SzSkeepDataSize(CSzData *sd, UInt64 size)
{
	if (size > sd->Size)
		return SZE_ARCHIVE_ERROR;
	sd->Size -= (size_t)size;
...
static SZ_RESULT SzSkeepData(CSzData *sd)
{
	UInt64 size;
...
static SZ_RESULT SzReadArchiveProperties(CSzData *sd)
{
	while(1)
	{
		UInt64 type;
...
static SZ_RESULT SzWaitAttribute(CSzData *sd, UInt64 attribute)
{
	while(1)
	{
		UInt64 type;
...
static SZ_RESULT SzReadBoolVector(CSzData *sd, size_t numItems, Byte **v, void * (*allocFunc)(size_t size))
{
	Byte b = 0;
	Byte mask = 0;
	size_t i;
...
static SZ_RESULT SzReadBoolVector2(CSzData *sd, size_t numItems, Byte **v, void * (*allocFunc)(size_t size))
{
	Byte allAreDefined;
	size_t i;
...
static SZ_RESULT SzReadHashDigests(
		CSzData *sd,
		size_t numItems,
		Byte **digestsDefined,
		UInt32 **digests,
...
static SZ_RESULT SzReadPackInfo(
		CSzData *sd,
...
static SZ_RESULT SzReadSwitch(CSzData *sd)
{
	Byte external;
...
static SZ_RESULT SzGetNextFolderItem(CSzData *sd, CFolder *folder, void * (*allocFunc)(size_t size))
{
	UInt32 numCoders;
	UInt32 numBindPairs;
...
static SZ_RESULT SzReadUnPackInfo(
		CSzData *sd,
		UInt32 *numFolders,
...
static SZ_RESULT SzReadSubStreamsInfo(
		CSzData *sd,
		UInt32 numFolders,
		CFolder *folders,
...
static SZ_RESULT SzReadStreamsInfo(
		CSzData *sd,
		CFileSize *dataOffset,
		CArchiveDatabase *db,
		UInt32 *numUnPackStreams,
...
static SZ_RESULT SzReadFileNames(CSzData *sd, UInt32 numFiles, CFileItem *files,
		void * (*allocFunc)(size_t size))
{
	UInt32 i;
...
static SZ_RESULT SzReadHeader2(
		CSzData *sd,
...
static SZ_RESULT SzReadAndDecodePackedStreams2(
...
static SZ_RESULT SzReadAndDecodePackedStreams(
		ISzInStream *inStream,
		CSzData *sd,
		CSzByteBuffer *outBuffer,
		CFileSize baseOffset,
...
static SZ_RESULT SzArchiveOpen2(
		ISzInStream *inStream,
		CArchiveDatabaseEx *db,
		ISzAlloc *allocMain,
		ISzAlloc *allocTemp)
{
	...
	if ( SafeReadDirect(inStream, signature, k7zSignatureSize) )
		return SZE_ARCHIVE_ERROR;
