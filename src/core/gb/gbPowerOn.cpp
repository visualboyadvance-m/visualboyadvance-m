// Power-on / post-boot RAM contents for GB / GBC.
//
// The base fill patterns and per-address difference tables reproduce
// real hardware power-up RAM dumps as shipped by the gambatte-core
// project (libgambatte/src/mem_dumps.h, GPLv2, by sinamas), which the
// gambatte hardware-test ROMs are calibrated against. The VRAM dump is
// the boot ROM's parting state (Nintendo logo tiles and tile map).
...
static const struct PowerOnDiff kDmgWramDiff[] = {
...
static const struct PowerOnDiff kCgbWramDiff[] = {
...
static const uint8_t kVramLogoDump[] = {
...
void gbPowerOnWramDmg(uint8_t* wram)
{
    ...
    for (size_t i = 0; i < sizeof(kDmgWramDiff) / sizeof(kDmgWramDiff[0]); ++i)
        wram[kDmgWramDiff[i].addr] = kDmgWramDiff[i].val;
}

// CGB: 32 KB of WRAM at power-on.
void gbPowerOnWramCgb(uint8_t* wram)
{
    for (unsigned addr = 0x0000; addr < 0x0800; addr += 0x10) {
        memset(wram + addr, 0xFF, 0x08);
        memset(wram + addr + 0x08, 0x00, 0x08);
    }
    for (unsigned addr = 0x0800; addr < 0x1000; addr += 0x10) {
        memset(wram + addr, 0x00, 0x08);
    ...
    for (size_t i = 0; i < sizeof(kCgbWramDiff) / sizeof(kCgbWramDiff[0]); ++i)
        wram[kCgbWramDiff[i].addr] = kCgbWramDiff[i].val;
}

// VRAM as the boot ROM leaves it: zeros, the Nintendo logo tile data
// (even bytes at $8010-$819F plus the (R) tile), and on DMG the logo
// tile map rows at $9904/$9924.
void gbPowerOnVram(uint8_t* vram, bool cgb, size_t vram_size)
{
    memset(vram, 0, vram_size);
    for (size_t i = 0; i < sizeof(kVramLogoDump); ++i)
        vram[0x0010 + i * 2] = kVramLogoDump[i];
    if (!cgb) {
        unsigned i = 1;
