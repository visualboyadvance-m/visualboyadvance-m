...
// Fractional prefetcher progress (see busPrefetchRomFloor in gbaCpu.h):
// busPrefetchAccum holds the cycles the prefetcher got into its next
// halfword during the current instruction's surplus; the CPU cores hand
// it off to busPrefetchHalfwords at the start of the next instruction,
// where a cart-bus data access turns it into an abort stall
// (busPrefetchRomStall).
...
// bus-release window) still observes the DMA's last bus value.
uint64_t g_eventBatchId = 0;
uint64_t g_dmaFireBatchId = ~0ull;
// Set when a DMA transfer ran; the first subsequent cart-bus access is
// forced non-sequential (the DMA disturbed the cart address latch).
bool g_dmaBrokeBurst = false;
// Overshoot of the instruction in flight across the last event boundary.
int g_eventBatchOvershoot = 0;
// Set by CPUCheckDMA for event-triggered (HBlank/VBlank) DMA dispatch:
// the DMA's bus grant waits for the in-flight instruction to complete.
bool g_dmaEventTriggered = false;
// The first post-DMA fetch slipped in before the DMA's bus grant; the
// burst break applies to the following cart access instead.
...
// Explicit "armed" boolean rather than an INT64_MIN sentinel: avoids any
// compiler-specific edge case around min-value sentinels (MSVC has been
// particular historically about how `-9223372036854775808` is parsed as
// negative-of-positive vs true negative literal). All arithmetic on the
// cycle stamp happens on always-positive values.
int64_t g_lastDispstatPollCycle = 0;
bool    g_haveLastDispstatPoll  = false;
...
void CPUUpdateFlags(bool breakLoop)
{
    ...
    bool prevIrqEnable = armIrqEnable;
    ...
    // Re-arm kSchedIrq on a false->true armIrqEnable transition. This
    // catches the IRQ-return path: SUBS pc,lr,#4 / LDMFD ^ restores
    // CPSR via CPUSwitchMode(...,breakLoop=false), so the breakLoop
    // gate isn't enough on its own. If a cascaded IRQ became pending
    // during the just-finished handler (typical for chained timers),
    // its kSchedIrq dispatch would have early-bailed because
    // armIrqEnable was false at the time -- re-test now so it's
    ...
    if (breakLoop) {
        ...
        // used to do for the polling-driven legacy delivery block.)
    }
}
...
void CPUSoftwareInterrupt(int comment)
{
    ...
    case 0x0C: {
        ...
        // The original `>> 5` was off by 4x (computing wordcount/32);
...
void doDMA(int ch, uint32_t& s, uint32_t& d, uint32_t si, uint32_t di, uint32_t c, int transfer32, bool isFIFO)
{
    ...
    //   subtests (Short/Trivial DMA x ARM/Thumb x IWRAM) confirm
    ...
    if (g_dmaEventTriggered) {
        ...
        // it cannot take the bus until the in-flight instruction
        // completes. The CPU-visible stall is therefore only the part of
        // (activation + transfer) that extends past the instruction's
        // overshoot of the trigger point. totalTicks was already charged
        // above; rebase it against the overshoot here.
        // The DMA is busy on the bus during [activation, activation +
        // transfer] after the trigger; the CPU keeps executing through
        // the activation latency and is only stalled for the part of the
...
{
    g_dmaEventTriggered = (reason == 1 || reason == 2);
    // DMA 0
...
void CPUUpdateRegister(uint32_t address, uint16_t value)
{
    ...
    case IO_REG_IF:
        ...
        // ever cancel a pending kSchedIrq, not schedule a new one.
        CPUTestIRQ();
        break;
    case IO_REG_WAITCNT: {
...
void CPUReset()
{
    ...
    holdState = false;
    holdType = 0;
    g_lastDispstatPollCycle = 0;
    g_haveLastDispstatPoll  = false;
...
// for filters that read past the screen
            ...
            // are now driven entirely by the kSchedIrq scheduler event:
            // every site that mutates IF/IE/IME calls CPUTestIRQ(), which
            // schedules a kSchedIrq at +8 cycles (or +3 inside the
            // IRQRecentTicks window for cascaded timer chains). The
            // dispatch handler in the scheduler pop-loop above clears
            // holdState/stopState/holdType unconditionally (matching
            // GBATEK halt-wake gating on (IE & IF) only) and then, if
            // IME and CPSR.I permit, calls CPUInterrupt(). The legacy
            // intState / IRQTicks fields are still serialized for save-
            // state compatibility but no longer drive control flow; they
            // are cleaned up on a follow-up save-state version bump.
