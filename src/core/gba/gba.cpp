...
// high bits but reads driven by that channel return 0 instead of the
// masked physical-memory contents — for both 16-bit and 32-bit transfers.
// Captured at write time because the mask makes the high halfword
// indistinguishable afterward. Cleared on the next DMA0SAD write that
// lands within the allowed range.
...
void doDMA(int ch, uint32_t& s, uint32_t& d, uint32_t si, uint32_t di, uint32_t c, int transfer32, bool isFIFO)
{
    ...
    if (transfer32) {
        ...
        if (s < 0x02000000) {
            ...
            // Channel 0 with original source out of allowed 27-bit range:
            // 16-bit reads return open-bus 0 instead of the masked
            // physical-memory contents.
            const bool ch0Oob16 = (ch == 0) && dma0SourceOOB;
            while (c != 0) {
                bool srcInROM = ((s >> 24) >= REGION_ROM0) && ((s >> 24) < REGION_SRAM);
                bool dstInROM = ((d >> 24) >= REGION_ROM0) && ((d >> 24) < REGION_SRAM);

                uint32_t value = ch0Oob16 ? 0u : CPUReadHalfWord(s);
...
void CPUUpdateRegister(uint32_t address, uint16_t value)
{
    ...
    case IO_REG_DMA0SAD_H:
        ...
        // were written so 16-bit and 32-bit reads can return open-bus 0
        // (matches real-HW DMA0 channel restriction to internal memory).
