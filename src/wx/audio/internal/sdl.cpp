...
void SDLAudio::write(uint16_t* finalWave, int length) {
    std::size_t samples = (length / audio.channels) / sizeof(uint16_t);
    std::size_t avail = 0;
...
#ifdef ENABLE_SDL3
    SDL_PutAudioStreamData(sound_stream, finalWave, length);
#else
    SDL_QueueAudio(sound_device, finalWave, length);
#endif
