...
EVT_HANDLER(wxID_ABOUT, "About...")
{
    ...
    // wxIconBundle::GetIcon() dereferences its ref data without checking it,
    // so it cannot be called on a bundle that was never given an icon.
    if (GetIcons().GetIconCount() != 0) {
