...
public:
    ...
    // Result from batch plugin enumeration
    struct PluginEnumResult {
        wxString path;
        wxString name;
        uint32_t flags;
        bool valid;
    ...
    // Batch enumerate plugins - validates all paths in a single IPC round trip.
    // Only sends paths that need the proxy (32-bit plugins on 64-bit build).
    // Returns results for each path with valid/invalid status and plugin info.
    // Uses a longer timeout since the host loads each plugin sequentially.
    std::vector<PluginEnumResult> EnumeratePlugins(const std::vector<wxString>& paths);
...
private:
    ...
    // Host ready event (signaled by host after initialization, replaces polling)
