...
enum class FilterCommand : uint32_t {
    ...
    EnumeratePlugins = 8  // Batch validate multiple plugin paths (no per-plugin IPC)
};
...
// Entry for batch plugin enumeration (stored in src buffer area)
struct PluginEnumEntry {
    wchar_t path[MAX_PATH];   // Input: full path to plugin
    char name[60];             // Output: plugin name (from RENDER_PLUGIN_INFO)
    uint32_t flags;            // Output: plugin flags
    uint32_t valid;            // Output: 1 = valid plugin, 0 = invalid
};

// Maximum plugins in a single batch enumeration
constexpr uint32_t kMaxEnumPlugins = 256;

// Ready event name template (signaled by host after initialization)
constexpr const wchar_t* kReadyEventNameFmt = L"Local\\VBAMRpiFilterReady_%u_%u";

inline void GetReadyEventName(uint32_t pid, uint32_t instance_id, wchar_t* buf, size_t buf_size) {
    swprintf_s(buf, buf_size, kReadyEventNameFmt, pid, instance_id);
}
...
struct SharedFilterBuffer {
    ...
    // For EnumeratePlugins command: number of plugin paths to validate
    // Entries are stored in the src buffer area as PluginEnumEntry[]
    uint32_t numEnumPlugins;
    ...
    // Helper to get enumeration entries array (stored in src buffer area)
    PluginEnumEntry* GetEnumEntries() {
        return reinterpret_cast<PluginEnumEntry*>(GetSrcBuffer());
    }

    const PluginEnumEntry* GetEnumEntries() const {
        return reinterpret_cast<const PluginEnumEntry*>(GetSrcBuffer());
    }
