...
bool RpiProxyClient::LaunchHost() {
    ...
    if (ready_event_) {
        ...
        if (waitResult == WAIT_OBJECT_0 &&
            ...
            ClientLog("LaunchHost: Host ready (via event)");
            return true;
        }
        ClientLog("LaunchHost: Ready event wait returned %lu, version=%u (expected %u)",
            waitResult, shared_mem_->version, SharedFilterBuffer::kCurrentVersion);
    }
    ...
    for (int i = 0; i < 10; i++) {
        if (shared_mem_->version == SharedFilterBuffer::kCurrentVersion) {
            ClientLog("LaunchHost: Host ready (via fallback poll)");
            return true;
        }
        Sleep(50);
    }
...
static const char* CommandToString(FilterCommand cmd) {
    switch (cmd) {
        ...
        case FilterCommand::EnumeratePlugins: return "EnumeratePlugins";
...
std::vector<RpiProxyClient::PluginEnumResult> RpiProxyClient::EnumeratePlugins(
    ...
    if (count > kMaxEnumPlugins) {
        ClientLog("EnumeratePlugins: too many plugins (%u > %u), truncating", count, kMaxEnumPlugins);
        count = kMaxEnumPlugins;
    }

    ClientLog("EnumeratePlugins: %u plugins to validate", count);

    if (!LaunchHost()) {
        ClientLog("EnumeratePlugins: LaunchHost failed");
        return results;
    }
    ...
    ClientLog("EnumeratePlugins: sending command (timeout=%u ms)", timeout);
    bool success = SendCommand(FilterCommand::EnumeratePlugins, timeout);
    LeaveCriticalSection(&main_channel_cs_);

    if (!success) {
        ClientLog("EnumeratePlugins: SendCommand failed");
        return results;
    }
    ...
    ClientLog("EnumeratePlugins: got %zu results", results.size());
    return results;
}
