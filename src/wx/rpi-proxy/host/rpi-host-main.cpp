...
static void HandleEnumeratePlugins() {
    ...
    Log("rpi-host: HandleEnumeratePlugins: %u plugins to validate\n", count);
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (LoadPluginInstance(entries[i].path, &handle, &info)) {
            ...
            entries[i].valid = 1;
            Log("rpi-host: EnumeratePlugins[%u]: valid, name='%s', flags=0x%x\n",
            ...
            entries[i].flags = 0;
            entries[i].valid = 0;
            Log("rpi-host: EnumeratePlugins[%u]: invalid\n", i);
        }
    }

    g_sharedMem->result = FilterResult::Success;
    Log("rpi-host: HandleEnumeratePlugins: done\n");
}
...
static bool ProcessCommand(FilterCommand cmd) {
    ...
    switch (cmd) {
        ...
        case FilterCommand::EnumeratePlugins:
            Log("rpi-host: Processing EnumeratePlugins\n");
...
int main(int argc, char* argv[]) {
    ...
    if (readyEvent) {
        ...
        Log("rpi-host: Signaled ready event\n");
    } else {
        Log("rpi-host: Could not open ready event (error %lu), client will use fallback polling\n", GetLastError());
    }
