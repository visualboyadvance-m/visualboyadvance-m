...
int FAGetDev(FAudio* fa) {
    const QString audio_device = OPTION(kSoundAudioDevice).Get();
    ...
    if (hr != 0) {
        vbam::LogError(Tr("FAudio: Enumerating devices failed!"));
        return 0;
    }
    ...
    for (uint32_t i = 0; i < dev_count; i++) {
        ...
        const QString device_id = config::utf16_to_utf8(dd.DeviceID);
...
bool FAudio_Output::init(long sampleRate) {
    ...
    if (hr != 0) {
        vbam::LogError(Tr("The FAudio interface failed to initialize!"));
        failed = true;
        return false;
    }
    ...
    if (hr != 0) {
        vbam::LogError(Tr("FAudio: Creating mastering voice failed!"));
        failed = true;
        return false;
    }
    ...
    if (hr != 0) {
        vbam::LogError(Tr("FAudio: Creating source voice failed!"));
        failed = true;
        return false;
    }
...
std::vector<AudioDevice> GetFAudioDevices() {
    ...
    for (uint32_t i = 0; i < dev_count; i++) {
        ...
        // Convert to UTF-8.
        const QString display_name = config::utf16_to_utf8(dd.DisplayName);
        const QString device_id = config::utf16_to_utf8(dd.DeviceID);
