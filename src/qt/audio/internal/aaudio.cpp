...
bool AAudioDriver::OpenStream() {
    ...
    if (AAudio_createStreamBuilder(&builder) != AAUDIO_OK || builder == nullptr) {
        vbam::LogMessage(QString::asprintf("AAudio: createStreamBuilder failed"));
        return false;
    }
    ...
    if (result != AAUDIO_OK || stream_ == nullptr) {
        vbam::LogMessage(QString::asprintf("AAudio: openStream failed: %s",
    ...
    if (AAudioStream_getFormat(stream_) != AAUDIO_FORMAT_PCM_I16 ||
        ...
        vbam::LogMessage(QStringLiteral("AAudio: unexpected stream format/channels; falling back"));
        CloseStream();
        return false;
    }
    ...
    if (start != AAUDIO_OK) {
        vbam::LogMessage(QString::asprintf("AAudio: requestStart failed: %s",
                            AAudio_convertResultToText(start)));
        CloseStream();
        return false;
    }

    vbam::LogMessage(QString::asprintf("AAudio: core %ld Hz, stream %ld Hz%s, burst %d, buffer %d/%d frames, "
                        "queue target %zu ms",
...
void AAudioDriver::ReopenIfDisconnected() {
    ...
    vbam::LogMessage(QString::asprintf("AAudio: stream disconnected, reopening"));
    CloseStream();
    OpenStream();
}
...
void AAudioDriver::LogHealth() {
    ...
    const size_t samples_per_ms = static_cast<size_t>(stream_rate_) * kChannels / 1000;
    vbam::LogDebug(QString::asprintf("AAudio: %u queue underruns (+%u), %d device xruns, queued %zu ms, "
                        "target %zu ms",
...
std::vector<AudioDevice> GetAAudioDevices() {
    // AAudio follows the system's output routing (speaker, headphones,
    // Bluetooth) itself; there is nothing to pick.
    return {AudioDevice{DefaultDeviceName(), QString()}};
}
