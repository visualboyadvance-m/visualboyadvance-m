...
OpenAL::~OpenAL() {
    ...
    // Wine incorrectly returns ALC_INVALID_VALUE and then fails the rest of
    // these functions as well, so there will be a leak under Wine, but that's
    // a bug in Wine, not this code.
...
bool OpenAL::init(long sampleRate) {
    ...
    if (!audio_device.isEmpty() && audio_device != audio::DefaultDeviceName()) {
        device = alcOpenDevice(audio_device.toUtf8().constData());
        if (device == nullptr) {
            // Might be the default device. Try again.
            OPTION(kSoundAudioDevice) = QString();
    ...
    if (!device) {
        vbam::LogError(QCoreApplication::translate("vbam", "OpenAL: Failed to open audio device"));
        return false;
    }
        ...
        if (!coreOptions.speedup && coreOptions.throttle && !gba_joybus_active) {
            ...
            while (nBuffersProcessed == 0) {
                ...
                std::this_thread::sleep_for(
                    std::chrono::milliseconds(soundBufferLen / (freq >> 7)));
                alGetSourcei(source, AL_BUFFERS_PROCESSED, &nBuffersProcessed);
                ASSERT_SUCCESS;
            }
        } else {
