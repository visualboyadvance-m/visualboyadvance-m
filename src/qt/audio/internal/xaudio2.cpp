...
#ifdef _MSC_VER
#include "qt/audio/internal/xaudio2_9.h"
#endif
...
HRESULT STDMETHODCALLTYPE XAudio2_Device_Notifier::OnDefaultDeviceChanged(EDataFlow flow, ERole, LPCWSTR pwstrDeviceId) {
    // Called on the MMDevice notification thread - no GUI calls permitted here.
    if (flow == eRender) {
...
void XAudio2_Device_Notifier::do_register(XAudio2_Output* p_instance) {
    ...
    LeaveCriticalSection(&instances_cs);
    vbam::LogError(Tr("XAudio2: Too many audio instances (max %1)").arg(MAX_INSTANCES));
}
...
void XAudio2_Device_Notifier::do_unregister(XAudio2_Output* p_instance, bool is_destructing) {
    ...
    for (int i = 0; i < MAX_INSTANCES; i++) {
        if (instances[i] == p_instance) {
            ...
            // already be partially torn down.
            if (!is_destructing)
...
void ScheduleXAudio2Reinit() {
    ...
    QMetaObject::invokeMethod(
        ...
        [] {
            soundShutdown();
            if (!soundInit()) {
                vbam::LogError(Tr("XAudio2: Failed to reinitialize audio after device change"));
            }
        },
        Qt::QueuedConnection);
}
...
struct XAudio2Context {
    ...
    bool Initialize() {
        ...
        if (hXAudio2) {
            ...
            hr = CoCreateInstance(CLSID_XAudio2_7, nullptr, CLSCTX_INPROC_SERVER, IID_IXAudio2_7, (void**)&xaudio2);
            if (SUCCEEDED(hr) && xaudio2) {
                ...
                if (SUCCEEDED(hr)) {
                    ...
                    vbam::LogError(Tr("XAudio2: XAudio2_7 Initialize failed with HRESULT: 0x%1")
                                       .arg(static_cast<unsigned long>(hr), 8, 16, QLatin1Char('0')));
                    xaudio2->Release();
                    xaudio2 = nullptr;
                }
            } else {
                vbam::LogError(Tr("XAudio2: Failed to create XAudio2 2.7"));
            }
        ...
        vbam::LogError(Tr("XAudio2: Could not load XAudio2_9.dll, XAudio2_8.dll, or XAudio2_7.dll!"));
        return false;
    }
};
...
std::vector<AudioDevice> GetXAudio2Devices() {
    ...
    if (g_xaudio2_context.version == XAudio2Version::XAudio2_7) {
        try {
            ...
            vbam::LogError(Tr("XAudio2: Failed to enumerate devices for XAudio2 2.7"));
            return {};
        }
    }
#ifdef _MSC_VER
