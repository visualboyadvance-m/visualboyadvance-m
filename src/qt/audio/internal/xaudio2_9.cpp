...
bool XAudio2_9_Output::SetupStereoUpmix() {
    ...
    if (matrixAvailable) {
        HRESULT hr = sVoice->SetOutputMatrix(nullptr, 2, outputChannels, matrix);
        if (FAILED(hr)) {
...
bool XAudio2_9_Output::init(long sampleRate) {
    ...
    if (!audio_device.isEmpty() && audio_device != audio::DefaultDeviceName()) {
        m_device_id = audio_device.toStdWString();
                ...
                if (SUCCEEDED(hr)) {
                    ...
                    if (pProps) {
                        ...
                        if (SUCCEEDED(hr) && varName.vt == VT_LPWSTR) {
                            devices.push_back({QString::fromWCharArray(varName.pwszVal),
                                               QString::fromWCharArray(pwszID)});
                        }
