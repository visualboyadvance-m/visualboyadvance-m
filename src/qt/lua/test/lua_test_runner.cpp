// Standalone smoke-test driver for the Qt frontend's Lua engine.
//
// Builds the LuaEngine + every lua_api_*.cpp binding without the Qt
// GUI, loads a Lua script (default: lua_api_test.lua next to this
// file), and runs it on a fresh coroutine. Prints engine log output to
// stdout and exits non-zero on any "[FAIL]" line or Lua error.
//
...
// vbam-core-fake target stubs everything except `coreOptions` and the
// frontend globals the bindings reference; supply those here so the
// link can complete.
