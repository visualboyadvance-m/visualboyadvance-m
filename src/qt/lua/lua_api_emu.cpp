...
//   emu.message(s)        — OSD message
//   emu.print(...)        — log to the Lua console
...
//
// emu.frameadvance is a yield point; the host loop pumps the script
...
// Globals from the Qt frontend (see qt/main-window.h) — declared here
// rather than included so the binding builds in the standalone test
// runner too.
...
// One-shot frame-yield flag — emu.frameadvance() sets it; the host
...
void LuaSetRomName(const std::string& name) { lua_internal::g_rom_name = name; }
