...
// in its own .cpp under src/qt/lua/ to keep this file manageable.
...
void LuaOnFrameBegin(uint8_t* pix, int width, int height, int pitch_bytes,
    ...
    //   5) (LuaOnFrameEnd) After the panel blits, run registerafter().
    e.ClearGui();
    lua_internal::g_frame_count++;
    e.ResumeScriptCoroutine();
    e.DispatchBeforeFrame();
    e.RenderOverlay(pix, width, height, pitch_bytes, bpp, r_shift, g_shift, b_shift);
}

void LuaOnFrameEnd() {
    LuaEngine& e = LuaInstance();
    if (e.IsLoaded()) e.DispatchAfterFrame();
}
...
bool LuaJoypadOverride(int joy, uint32_t* mask) {
    ...
    uint32_t lua_mask = 0;
    if (!e.GetJoypadOverride(joy, &lua_mask)) return false;
    if (mask) *mask = lua_mask;
    return true;
}
...
void LuaOnGameUnload() {
    ...
    if (e.IsLoaded()) {
        e.Stop();
        e.Log("[lua] script stopped (game unloaded)");
    }
}
