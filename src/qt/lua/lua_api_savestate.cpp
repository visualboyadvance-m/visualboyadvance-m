...
// string — a slot number 0–9 (matching the frontend's quick-state slots) or an
// arbitrary path for user-named saves. Both vbam GB and GBA cores use
// gz-compressed savestates that the Load/Save State menu items
// already write/read; we plug into the same paths so a
...
std::string SlotPath(const std::string& slot) {
    // If the caller passed a digit slot (0..9), expand to a quick-state
    // file; otherwise treat as a literal path. The frontend's own
    // quick-state paths are constructed by its command handlers —
    // Lua-driven saves bypass that to keep the binding independent of
    // GUI headers, so a digit just becomes ".sav<n>".
