// Lua 5.4 scripting host for the Qt frontend.
//
// One LuaEngine instance owns one lua_State; the rest of the frontend
...
//   emu.print(...)        — writes to the Lua console window
...
// (lua_api_*.cpp); this header declares the engine itself plus the
// frontend integration points (bottom of the file), which are inline
// no-ops when VBAM_ENABLE_LUA is not defined so call sites in
// sys.cpp / game-area.cpp need no #ifdef.

#ifndef VBAM_QT_LUA_LUA_ENGINE_H_
#define VBAM_QT_LUA_LUA_ENGINE_H_
...
// LoadStringW or LoadStringA. Qt headers may transitively include
// <windows.h>, so any TU that includes them before this one would
// otherwise see LuaEngine::LoadString rewritten. Undefine the macro
// before the class so the method name is consistent across callers.
#ifdef LoadString
#  undef LoadString
#endif
...
class LuaEngine {
public:
    // Callback the console window registers so script `print()` /
    ...
    // at the call site (core/base/script_hooks.h trampolines).
    ...
    // joypad.set() override. The joypad polling code asks for the
    ...
    // Savestate hook dispatchers. Pass the slot or filename that's
    // being saved/loaded; scripts get a Lua representation via
    // savestate.create().
    ...
    // frame hook reads it after the frame is composed and overlays it.
...
// process exit. The emulator core can only run one ROM at a time, so
// one engine is enough.
LuaEngine& LuaInstance();

// ---------------------------------------------------------------------
// Frontend integration points. These are the exact call sites the wx
// port has in sys.cpp; the Qt port's sys.cpp / game-area.cpp call
// these and nothing else, so they need no #ifdef (see the #else below).
// ---------------------------------------------------------------------

// Call at the top of systemDrawScreen(), BEFORE the frame is handed to
// the drawing panel (panel->DrawArea(&g_pix)). Clears last frame's
// gui.* queue, bumps emu.framecount(), resumes the script coroutine
// (emu.frameadvance), runs registerbefore() callbacks and composes the
// gui.* overlay into `pix`. No-op when no script is loaded. Arguments
// are those of RenderOverlay(); the wx port passes:
//   width  = gbRom ? 160 : 240,  height = gbRom ? 144 : 160,
//   pitch  = (gbRom ? 161 : 241) * (systemColorDepth / 8),
//   systemColorDepth, systemRedShift, systemGreenShift, systemBlueShift.
void LuaOnFrameBegin(uint8_t* pix, int width, int height, int pitch_bytes,
                     int bpp, int r_shift, int g_shift, int b_shift);

// Call at the end of systemDrawScreen(), AFTER panel->DrawArea(). Runs
// registerafter() callbacks. No-op when no script is loaded.
void LuaOnFrameEnd();

// Call in systemReadJoypad(joy) after the frontend computed `*mask`
// (already masked with REALKEY_MASK) and before game recording sees it.
// Replaces *mask with the script's joypad.set() override for `joy` and
// returns true if one is installed; KEYM_* bits match FCEUX bit
// positions so the mask slots straight in (caller re-applies
// REALKEY_MASK). Returns false (mask untouched) otherwise.
bool LuaJoypadOverride(int joy, uint32_t* mask);

// Call from GameArea::LoadGame() with the ROM's base name (no
// extension) and from UnloadGame() with "" — backs emu.romname().
void LuaSetRomName(const std::string& name);

// Call from GameArea::UnloadGame(): stops the running script (fires
// registerexit) so no hook touches a core that is being torn down.
void LuaOnGameUnload();

#else  // !VBAM_ENABLE_LUA

inline void LuaOnFrameBegin(uint8_t*, int, int, int, int, int, int, int) {}
inline void LuaOnFrameEnd() {}
inline bool LuaJoypadOverride(int, uint32_t*) { return false; }
inline void LuaSetRomName(const std::string&) {}
inline void LuaOnGameUnload() {}
...
#endif  // VBAM_QT_LUA_LUA_ENGINE_H_
