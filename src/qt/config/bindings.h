#ifndef VBAM_QT_CONFIG_BINDINGS_H_
#define VBAM_QT_CONFIG_BINDINGS_H_
...
public:
    ...
    // configuration.
    std::vector<std::pair<QString, QString>> GetKeyboardConfiguration() const;

    // Returns the game control configuration for the INI file. These go in the
    // [Joypad] section of the INI file.
    std::vector<std::pair<GameCommand, QString>> GetJoypadConfiguration() const;
...
#endif  // VBAM_QT_CONFIG_BINDINGS_H_
