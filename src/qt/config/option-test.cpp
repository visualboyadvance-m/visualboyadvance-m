...
TEST(OptionTest, Double) {
    ...
    EXPECT_TRUE(option->command().isEmpty());
    ...
    const vbam::LogNull disable_logging;
...
TEST(OptionTest, Enum) {
    ...
    EXPECT_TRUE(option->SetRenderMethod(config::RenderMethod::kOpenGL));
    EXPECT_EQ(option->GetRenderMethod(), config::RenderMethod::kOpenGL);
    EXPECT_EQ(option->GetEnumString(), "opengl");

    EXPECT_TRUE(option->SetEnumString("simple"));
    EXPECT_EQ(option->GetRenderMethod(), config::RenderMethod::kSimple);
