...
TEST(BindingsTest, Default) {
    ...
    inputs = bindings.InputsForCommand(config::ShortcutCommand(cmd::kClose));
    EXPECT_TRUE(inputs.find(config::KeyboardInput('W', config::kKeyModControl)) != inputs.end());

    inputs = bindings.InputsForCommand(config::ShortcutCommand(cmd::kLoadGame01));
    EXPECT_TRUE(inputs.find(config::KeyboardInput(Qt::Key_F1)) != inputs.end());
...
TEST(BindingsTest, AssignDefault) {
    ...
    bindings.AssignInputToCommand(config::KeyboardInput(Qt::Key_F1),
                                  config::ShortcutCommand(cmd::kClose));
...
TEST(BindingsTest, UnassignDefault) {
    ...
    // Unassign F1.
    bindings.UnassignInput(config::KeyboardInput(Qt::Key_F1));
...
TEST(BindingsTest, ReassignDefault) {
    ...
    bindings.AssignInputToCommand(config::KeyboardInput(Qt::Key_F1),
                                  config::ShortcutCommand(cmd::kLoadGame01));
...
TEST(BindingsTest, AssignToNoop) {
    ...
    bindings.AssignInputToCommand(config::KeyboardInput(Qt::Key_F1),
                                  config::ShortcutCommand(cmd::kNoop));

    const auto command = bindings.CommandForInput(config::KeyboardInput(Qt::Key_F1));
...
TEST(BindingsTest, AssignUnusedToNoop) {
    ...
    bindings.AssignInputToCommand(config::KeyboardInput('T'), config::ShortcutCommand(cmd::kNoop));
...
TEST(BindingsTest, AssignDefaultToGame) {
    ...
    // "Up" command.
    bindings.AssignInputsToCommand({config::KeyboardInput(Qt::Key_F1)},
    ...
    EXPECT_EQ(
        bindings.InputsForCommand(config::GameCommand(config::GameJoy(0), config::GameKey::Up)),
        std::unordered_set<config::UserInput>{config::KeyboardInput(Qt::Key_F1)});

    EXPECT_EQ(bindings.CommandForInput(config::KeyboardInput(Qt::Key_F1)),
...
TEST(BindingsTest, ClearCommand) {
    ...
    bindings.ClearCommandAssignments(config::ShortcutCommand(cmd::kClose));
    ...
    EXPECT_TRUE(bindings.InputsForCommand(config::ShortcutCommand(cmd::kClose)).empty());
}

// Every joypad game command must appear in the joypad configuration, in order.
TEST(BindingsTest, JoypadConfigurationOrder) {
    const config::Bindings bindings;
    const auto config = bindings.GetJoypadConfiguration();
    ASSERT_EQ(config.size(), config::kNbGameKeys * config::kNbJoypads);
    EXPECT_EQ(config[0].first.ToConfigString(), "Joypad/1/Up");
    EXPECT_EQ(config[config::kNbGameKeys].first.ToConfigString(), "Joypad/2/Up");
    EXPECT_EQ(config.back().first.ToConfigString(), "Joypad/4/GS");
}
