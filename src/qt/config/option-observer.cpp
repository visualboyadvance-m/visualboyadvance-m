...
public:
    CallbackOptionObserver(const OptionID& option_id, std::function<void(Option*)> callback)
...
OptionsObserver::OptionsObserver(const OptionID& option_id, std::function<void(Option*)> callback)
...
OptionsObserver::OptionsObserver(const std::vector<OptionID>& option_ids,
    ...
    for (const OptionID& option_id : option_ids) {
        observers_.emplace(std::make_unique<CallbackOptionObserver>(option_id, callback));
    }
}
