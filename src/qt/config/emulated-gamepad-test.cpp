...
TEST_F(GamepadTest, Basic) {
    ...
    EXPECT_EQ(gamepad()->GetJoypad(0), 0u);
...
TEST_F(GamepadTest, UnassignedInput) {
    const config::KeyboardInput f1(Qt::Key_F1);
    const config::JoyInput up(config::JoyId(0), config::JoyControl::HatNorth, 0);
...
// SetGameKey bypasses the bindings entirely.
TEST_F(GamepadTest, SetGameKey) {
    gamepad()->SetGameKey(config::GameJoy(0), config::GameKey::Up, true);
    EXPECT_EQ(gamepad()->GetJoypad(0), KEYM_UP);
    gamepad()->SetGameKey(config::GameJoy(0), config::GameKey::Up, false);
    EXPECT_EQ(gamepad()->GetJoypad(0), 0u);
    EXPECT_EQ(gamepad()->GetJoypad(99), 0u);
}
