...
std::vector<std::pair<QString, QString>> Bindings::GetKeyboardConfiguration() const {
...
std::vector<std::pair<GameCommand, QString>> Bindings::GetJoypadConfiguration() const {
    std::vector<std::pair<GameCommand, QString>> config;
    config.reserve(kNbGameKeys * kNbJoypads);

    for (const auto& game_command : internal::OrderedGameCommands()) {
        const auto iter = control_to_inputs_.find(Command(game_command));
        if (iter == control_to_inputs_.end()) {
            config.push_back(std::make_pair(game_command, QString()));
            continue;
        }
