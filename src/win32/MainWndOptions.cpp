...
{
    pCmdUI->SetCheck(
        (throttle != 0) && (throttle != 25) && (throttle != 50) && (throttle != 100) && (throttle != 150) && (throttle != 200));
}
...
void MainWnd::OnOptionsVideoFullscreen()
{
    ...
    if (theApp.display->selectFullScreenMode(mode)) {
        if ((mode.width != fsWidth) || (mode.height != fsHeight) || (mode.bitDepth != fsColorDepth) || (mode.frequency != fsFrequency) || (mode.adapter != fsAdapter) || (videoOption != VIDEO_OTHER)) {
            fsForceChange = true;
            fsWidth = mode.width;
