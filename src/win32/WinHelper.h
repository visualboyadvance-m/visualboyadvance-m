...
public:
    inline CSize(){};
    inline explicit CSize(const SIZE& size)
    {
        cx = size.cx;
        cy = size.cy;
    }
    inline explicit CSize(long nSizeX, long nSizeY)
    {
        cx = nSizeX;
        cy = nSizeY;
    }
    inline void Set(long nSizeX, long nSizeY)
    {
        cx = nSizeX;
        cy = nSizeY;
    }
    ...
    inline CSize& operator=(const SIZE& size)
    {
        cx = size.cx;
        cy = size.cy;
        return *this;
...
public:
    ...
    inline explicit CRect(const RECT& rhs)
    {
        Set(rhs.left, rhs.top, rhs.right, rhs.bottom);
    }
    inline CRect(int xLeft, int yTop, int xRight, int yBottom)
    {
    ...
    inline bool PtInRect(const POINT& pt) const
    {
    ...
    {
        return (rc.left < right && rc.right > left && rc.top < bottom && rc.bottom > top);
    }
};
...
public:
    inline CPoint(){};
    inline CPoint(LPARAM lParam)
    {
        x = LOWORD(lParam);
        y = HIWORD(lParam);
    }
    inline CPoint(int nX, int nY)
    {
        x = nX;
        y = nY;
    }
    ...
    inline bool operator==(const CPoint& rhs) const
    {
...
public:
    ...
    {
    public:
        inline CLock(CCriticalSection& sect)
            : m_sect(sect)
        {
            m_sect.Lock();
        }
...
//      Load a 16x16 icon from the same resource as the other size icons.
{
    return reinterpret_cast<HICON>(
        ::LoadImage(hInst, MAKEINTRESOURCE(uID), IMAGE_ICON, 16, 16, LR_DEFAULTCOLOR));
}
...
{
public:
    inline CDeferWindowPos(const int nWindows = 1)
        : m_hdlDef(::BeginDeferWindowPos(nWindows))
    {
    }
    inline ~CDeferWindowPos()
    {
        R_VERIFY(::EndDeferWindowPos(m_hdlDef));
    }
    inline HDWP DeferWindowPos(HWND hWnd, HWND hWndInsertAfter, int x, int y, int cx, int cy,
        UINT uFlags)
    {
    ...
    {
        return ::DeferWindowPos(m_hdlDef,
            hWnd,
            hWndInsertAfter,
            rc.left,
            rc.top,
