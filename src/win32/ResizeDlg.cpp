...
void AssertFailed(char* file, int line, char* exp)
{
    ...
    int res = MessageBox(*theApp.m_pMainWnd, buffer, "Assertion failed!",
        MB_ICONHAND | MB_SETFOREGROUND | MB_TASKMODAL | MB_ABORTRETRYIGNORE);
...
void ApiFailure(char* pcszFilename, int nLine, char* pcszExpression)
{
    ...
    char szMessage[1024];
    _snprintf(szMessage, countof(szMessage), "API VERIFY Failure!"
                                             "\nProgram: %s"
                                             "\n"
                                             "\nFile %s"
                                             "\nLine %d"
        ...
        szExeName, pcszFilename, nLine, pcszExpression, dwLastError, lpMsgBuf);
...
//    It will return non-zero for success and zero if it fails
{
    ...
    if (::IsWindow(*this)
        && psd
        && ((hkRootSave != NULL && pcszName != NULL && !IsBadStringPtr(pcszName, 0xFFFF)) || (hkRootSave == NULL && pcszName == NULL))
