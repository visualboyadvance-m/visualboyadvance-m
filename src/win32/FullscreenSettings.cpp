...
void FullscreenSettings::OnCbnSelchangeComboColorDepth()
{
        ...
        for (UINT i = 0; i < nModes; i++) {
            ...
            if (res == D3D_OK) {
                ...
                for (UINT surf = 0; surf < i; surf++) {
                    ...
                    if ((allModes[surf].Width == mode.Width) && (allModes[surf].Height == mode.Height)) {
                        alreadyPresent = true;
                    }
                }
    ...
    if (api == OPENGL) {
        ...
        while (TRUE == EnumDisplaySettings(devInfo.DeviceName, iMode, &mode)) {
            if ((mode.dmBitsPerPel != selectedBPP) || (mode.dmDisplayFlags != 0)) {
                iMode++;
                continue;
            }
            alreadyExists = false;
            for (int iItem = 0; iItem < combo_resolution.GetCount(); iItem++) {
                ...
                if ((mode.dmPelsWidth == surfMode.dmPelsWidth) && (mode.dmPelsHeight == surfMode.dmPelsHeight)) {
                    alreadyExists = true;
                    break;
                }
            }
...
void FullscreenSettings::OnCbnSelchangeComboResolution()
{
    ...
    if (api == DIRECT_3D) {
        ...
        for (UINT i = 0; i < nModes; i++) {
            ...
            if ((res == D3D_OK) && (mode2.Width == mode.Width) && (mode2.Height == mode.Height)) {
                CString temp;
    ...
    if (api == OPENGL) {
        ...
        while (TRUE == EnumDisplaySettings(devInfo.DeviceName, iMode, &mode)) {
            if ((mode.dmBitsPerPel != selectedResolution.dmBitsPerPel) || (mode.dmDisplayFlags != 0) || (mode.dmPelsWidth != selectedResolution.dmPelsWidth) || (mode.dmPelsHeight != selectedResolution.dmPelsHeight)) {
                iMode++;
                continue;
            }
            alreadyExists = false;
...
void FullscreenSettings::OnOK()
{
    ...
    if (api == OPENGL) {
        ...
        switch (mode.dmBitsPerPel) { // the policy of this dialog is to return the actually usable amount of bits
        case 4:
            m_colorDepth = 4;
            break;
        case 8:
