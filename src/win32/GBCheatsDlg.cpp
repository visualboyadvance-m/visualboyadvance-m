...
{
    if (GetDlgItem(IDC_UPDATE)->SendMessage(BM_GETCHECK, 0, 0) & BST_CHECKED)
        updateValues = true;
    else
        updateValues = false;
...
BOOL GBCheatSearch::OnInitDialog()
{
    ...
    updateValues = regQueryDwordValue("gbCheatsUpdate", 0) ? true : false;
...
void GBCheatSearch::addChanges(bool showMsg)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        item.iItem = i;
        item.iSubItem = 0;
        item.lParam = data[i].addr | (data[i].bank << 16);
        item.state = 0;
        item.stateMask = 0;
...
void GBCheatList::OnItemchangedCheatList(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    if (!duringRefresh) {
        ...
        if (l->uChanged & LVIF_STATE) {
            if (((l->uOldState & LVIS_STATEIMAGEMASK) >> 12) != (((l->uNewState & LVIS_STATEIMAGEMASK) >> 12))) {
